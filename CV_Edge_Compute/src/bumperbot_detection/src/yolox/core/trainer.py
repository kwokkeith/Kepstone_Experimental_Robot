...
class Trainer:
    ...
    def after_iter(self):
        ...
        if (self.global_iter + 1) % self.exp.eval_interval == 0:
            self.save_ckpt(ckpt_name="latest")
            all_reduce_norm(self.model)
            self.evaluate_and_save_model()
    ...
    def evaluate_and_save_model(self):
        ...
        if self.rank == 0:
            if self.args.logger == "tensorboard":
                self.tblogger.add_scalar("val/COCOAP50", ap50, self.global_iter + 1)
                self.tblogger.add_scalar("val/COCOAP50_95", ap50_95, self.global_iter + 1)
            if self.args.logger == "wandb":
                self.wandb_logger.log_metrics({
                    "val/COCOAP50": ap50,
                    "val/COCOAP50_95": ap50_95,
                    "train/epoch": self.epoch + 1,
                    "train/global_iter": self.global_iter + 1,
                })
            ...
            if self.args.logger == "mlflow":
                ...
                self.mlflow_logger.on_log(self.args, self.exp, self.global_iter+1, logs)
            logger.info("\n" + summary)
        synchronize()

        self.save_ckpt("last_epoch", update_best_ckpt, ap=ap50_95)
        if self.save_history_ckpt:
            self.save_ckpt(f"iter_{self.global_iter + 1}", ap=ap50_95)
            # self.save_ckpt(f"epoch_{self.epoch + 1}", ap=ap50_95)

        if self.args.logger == "mlflow":
            metadata = {
                    "epoch": self.epoch + 1,
                    "global_iter": self.global_iter + 1,
                    "input_size": self.input_size,
                    'start_ckpt': self.args.ckpt,
                    'exp_file': self.args.exp_file,
                    "best_ap": float(self.best_ap)
    ...
    def save_ckpt(self, ckpt_name, update_best_ckpt=False, ap=None):
        if self.rank == 0:
            ...
            ckpt_state = {
                ...
                "start_global_iter": self.global_iter + 1,
            ...
            if self.args.logger == "wandb":
                self.wandb_logger.save_checkpoint(
                    ...
                    metadata={
                        "epoch": self.epoch + 1,
                        "iter_global": self.global_iter + 1,
                        "optimizer": self.optimizer.state_dict(),
                        "best_ap": self.best_ap,
                        "curr_ap": ap
                    }
