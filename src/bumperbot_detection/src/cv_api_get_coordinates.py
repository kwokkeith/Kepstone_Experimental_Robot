...
class BallDetector:
    def __init__(self):
        ...
        self.intrinsics = None  # Placeholder for intrinsics
        ...
        # Subscribe to camera info topic to get intrinsics
        rospy.Subscriber("/camera/depth/camera_info", CameraInfo, self.camera_info_callback)

        # Synchronize the color and depth messages
        self.ts = message_filters.ApproximateTimeSynchronizer([self.color_sub, self.depth_sub], 10, 0.5)
    ...
    def camera_info_callback(self, msg):
        ...
        rospy.loginfo(f"Intrinsics: {self.intrinsics}")


    def image_callback(self, color_msg, depth_msg):
        try:
            print("Received new frame!")  # Debugging
            
            # Convert ROS image messages to OpenCV images
            color_image = self.bridge_object.imgmsg_to_cv2(color_msg, "bgr8")
            depth_image = self.bridge_object.imgmsg_to_cv2(depth_msg, "16UC1")  # Use 16-bit encoding

            # depth_array = np.array(depth_image, dtype=np.float32)

            # # Debugging: Print min/max depth
            # print(f"Min Depth: {np.min(depth_array)}, Max Depth: {np.max(depth_array)}")

            # # Normalize depth image for visualization
            # if np.max(depth_array) > 0:
            #     depth_norm = cv2.normalize(depth_array, None, 0, 255, cv2.NORM_MINMAX)
            #     depth_norm = np.uint8(depth_norm * (255.0 / np.max(depth_norm)))  # Ensure scaling
            # else:
            #     depth_norm = np.zeros_like(depth_array, dtype=np.uint8)  # Avoid division by zero

            # depth_colormap = cv2.applyColorMap(depth_norm, cv2.COLORMAP_JET)  # Apply colormap

            # # Resize depth map to match color image dimensions (optional)
