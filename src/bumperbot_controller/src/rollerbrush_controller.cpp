    ...
    current_power_(0.0)
{
    ...
    set_rollerbrush_service_ = nh_.advertiseService(set_rollerbrush_power_srv_svr_, &RollerBrushController::setRollerBrushPower, this);
    get_rollerbrush_service_ = nh_.advertiseService(get_rollerbrush_power_srv_svr_, &RollerBrushController::getRollerBrushPower, this);
    // Initialize Topic Publishers
    rollerbrush_pub_ = nh_.advertise<std_msgs::Float32>(rollerbrush_power_topic_pub_, 10);


    // Publish default rollerbrush power (0.0)
    std_msgs::Float32 msg;
    msg.data = current_power_;
    rollerbrush_pub_.publish(msg);
