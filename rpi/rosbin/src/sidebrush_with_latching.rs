...
pub fn main() {
    ...
    let mut spd_latch = Arc::new(AtomicBool::new(false));
    let mut time = Instant::now();
    let mut time_latch = false;
    ...
    let sub_spd_latch = spd_latch.clone();
    ...
    let _subscriber_raii = rosrust::subscribe("/sidebrush_controller/sidebrush_speed", 100, move |v: rosrust_msg::std_msgs::UInt32| {
        ...
        if !sub_spd_latch.load(Ordering::Relaxed) {
            speed.clone().store(v.data, Ordering::Relaxed);
            if v.data > 0 {
                sub_spd_latch.store(true, Ordering::Relaxed);
            }
        }
    }).unwrap();
    ...
    while rosrust::is_ok() {
        ...
        if time.elapsed().as_secs() == 10 && spd_latch.load(Ordering::Relaxed) {
            println!("Unlatch");
            spd_latch.store(false, Ordering::Relaxed);
            time_latch = false;
        }
        if current_speed == target_speed && spd_latch.load(Ordering::Relaxed) && time_latch == false {
            println!("Latching {}RPM", &current_speed);
